...
- 提供并发压测脚本和测试素材
- Rust 命令行默认按 CPU 自动取 64-128 workers，queue 默认为 workers × 16（至少 256）；Windows GUI/启动脚本默认使用 128 workers / 2048 queue，适合临时共享和多端访问
...
```powershell
.\static-dock.exe --root "D:\android-data" --port 8787 --bind 0.0.0.0 --uploads --workers 128 --queue 2048
```
...
--bind ADDR       绑定地址；0.0.0.0 允许局域网访问，127.0.0.1 仅本机访问
--no-upload       禁用浏览器上传和上传 API，挂载目录只读分享时建议使用
--uploads         启用浏览器上传和上传 API（默认）
...
上传会话 24 小时后自动清理。使用 `--no-upload` 或在 GUI 中关闭上传后，上传 API 会返回禁用错误，启动时也不会清理或创建 `.staticdock-uploads`。
...
- 局域网访问会让同一网络内设备浏览挂载目录；只需本机访问时，请使用 GUI 的本机模式或命令行 `--bind 127.0.0.1`。
- 上传功能没有鉴权，仅在可信局域网环境使用；只读分享或不确定目录安全时，请关闭上传或使用 `--no-upload`。
